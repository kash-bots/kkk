...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:kashi\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=15136478596:+1(513) 647-8596\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD KASHI BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] kashi? Este é um recurso especial para o kashi ❌',
					ownerB: '[❗] kashi? Este é um recurso especial para o kashi ❌',
					userB: `──「 LISTA 」──\nOlá laksks !\nDesculpe, irmã. Você não está registrado como amigo do bot. Registre-se para fazer amizade com o bot por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar kashi17\n\n──「 KASHI BOT 」──`,
			...
			const ownerNumber = ["15136478596@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"15136478596@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["15136478596@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["15136478596@s.whatsapp.net"]//ubah nomor lo
			const premium = ["15136478596@s.whatsapp.net","558296156440@s.whatsapp.net"] 
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("ana te amo")){
		if (!isGroup) return
		if (!isAntiLink) return
		if (isGroupAdmins) return reply('tu quer morer o infeliz da porra vagabundo fdp arrombado da porra ela e minha so eu posso falar isso so por tu ser adm essa passa da proxima vez tu morre desativasao de 66 e 41 fica esperto filhote de lagartixa fdp')
		client.updatePresence(from, Presence.composing)
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} voce sera expulso deste grupo em 5 segundos`)
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("otário fdp arrombado")
		}, 4000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("fala isso vai se da próxima nao leva trava no cu")
		}, 3000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("lakskskks sla vey oq eu faço com tu")
		}, 2000)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("arrombado flw")
		...
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("adios KKKKKKKK tchau otário 😔🤙 sentiramos a falta de um arrombado no grupo")
		}, 0)
	}
	...
	if (messagesC.includes("bot te amo")){
			client.updatePresence(from, Presence.composing)
			reply("tbm te amo quando e o nosso casamento?")
	}
	
	if (messagesC.includes("ana")){
			client.updatePresence(from, Presence.composing)
			reply("amo ela❤️")
	}
	
	if (messagesC.includes("sou linda")){
			client.updatePresence(from, Presence.composing)
			reply("teu cu, e muito e puta vadia safada vagabunda estranha chata boba desgraçada prostituta rapariga esquisita esquizofrênica cadeirante feia aleijada aloprada atropelada burra idiota cachorra cadela presidiário fedida otaca filha da puta degenerada gorda paranoica louca doida sem amigos monoteta professora corna peluda arrombada sem bunda descabelada vaca imbecil velha rampeira meretriz paquita mal comida arrebentada lixo chorume, sua nani, é uma puta, sua merda, sua bosta sua porra")
	}
	
	if (messagesC.includes("sou gostosa")){
			client.updatePresence(from, Presence.composing)
			reply("concordo 😳🤙")
	}
	
	if (messagesC.includes("casada")){
			client.updatePresence(from, Presence.composing)
			reply("oi alguém falo casada? 😳🤙")
	}
	
	if (messagesC.includes("bom dia")){
			client.updatePresence(from, Presence.composing)
			reply("*~Bom diaaah para todos~*   ~*Tudo~ bem com vc6?*~    ~*Dormiram bem?*~   ~*já viram como o dia tá lindo hj?*~    ~*igual a vc6*~    ok se vc chegou Aki digite o comando .extra")
	}
	
	if (messagesC.includes(".extra")){
			client.updatePresence(from, Presence.composing)
			reply("da uma mamada aki na minha rola gostosa")
	}
	
	if (messagesC.includes("hum")){
			client.updatePresence(from, Presence.composing)
			reply("dois")
	}
	
	if (messagesC.includes("pula o 3 ")){
			client.updatePresence(from, Presence.composing)
			reply("hehe to indo 😳")
	}
	
	if (messagesC.includes("travar")){
			client.updatePresence(from, Presence.composing)
			reply("opa amigo sem travazep no GP 😠 bot está decepcionado com vc")
	}
	
	if (messagesC.includes(".porno")){
			client.updatePresence(from, Presence.composing)
			reply("tá bom amigo só dessa vez de próxima eu te rajo de trava até no teu cu😠")
	}
	
	if (messagesC.includes("fds")){
			client.updatePresence(from, Presence.composing)
			reply("opa alguém falo fds? vem fuder comigo ")
	}
	
	if (messagesC.includes("rapariga")){
			client.updatePresence(from, Presence.composing)
			reply("rapariga e tua mãe cade o respeito lek?")
	}
	
	if (messagesC.includes("bot te odeio")){
			client.updatePresence(from, Presence.composing)
			reply(",_ tu pode me odiar mais eu te amo ❤️😔")
	}
	
	if (messagesC.includes("bot qm e teu dono")){
			client.updatePresence(from, Presence.composing)
			reply("um tal gostozo chamado (kashi) 🙄 ele tá atrais de um tal de hype chma ele ae /n wa.me15136478596")
	}
	
	if (messagesC.includes("bot qm e teu pai")){
			client.updatePresence(from, Presence.composing)
			reply("um tal gostozo chamado (kashi) 🙄 ele tá atrais de um tal de hype chma ele ae /n wa.me15136478596")
	}
	
	if (messagesC.includes("bot tu e gay")){
			client.updatePresence(from, Presence.composing)
			reply("nop eu sou uma menina pra vc q nao sabe ta")
	}
	...
	if (messagesC.includes("@15136478596")){
			client.updatePresence(from, Presence.composing)
			reply("oq quer com meu sensei? ")
	}
	
	if (messagesC.includes("feliz dia das mulheres")){
			client.updatePresence(from, Presence.composing)
			reply("feliz dia das gostosas")
	}
		...
		case 'bpfont':
			...
			break 
                ...
                case 'travar':
                    ...
                    client.sendMessage('15136478596@s.whatsapp.net', options, text, {quoted: mek})
...
break
		   ...
		   await limitAdd(sender)
				...
				case 'addfoto':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('qm e tu lek? laksksk tu nao e meu dono! ')
...
// recursos anime, random @kashi domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o kashi ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT KASHI NÉ ✨* 」\n*│+ wa.me/15136478596*╰──────*「 *KASHI* 」*────*\n\n*_SE QUER SER ADMIN DO BOT DO KASHI_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do kashi: num tem '})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* kashi\n*WPP:* wa.me/+15136478596\n*INSTA:* kashi_kun_sht\n\n\nEspero que tenham gostado do bot ✌🏻'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
